...
class HCRestartCleanupSpec extends TestKitBaseClass with FixtureAnyFunSuiteLike with HCStateTestsHelperMethods {
  ...
  test("List pending outgoing HTLCs for NORMAL and CLOSED channel") { f =>
    ...
    awaitCond(aliceHC.params.asInstanceOf[CustomCommitmentsPlugin].getHtlcsRelayedOut(Nil).values.flatten.map(_._2).toSet == Set(id1, id2, id3, id4))

    fulfillAliceHtlcByBob(alice2bobUpdateAdd1.id, preimage1, f)
    fulfillAliceHtlcByBob(alice2bobUpdateAdd2.id, preimage2, f)

    awaitCond(aliceHC.params.asInstanceOf[CustomCommitmentsPlugin].getHtlcsRelayedOut(Nil).values.flatten.map(_._2).toSet == Set(id3, id4))

    val (_, cmd_add5, _) = makeCmdAdd(1000000L.msat, bobKit.nodeParams.nodeId, currentBlockHeight)
    alice ! cmd_add5
    val aliceAdd5 = alice2bob.expectMsgType[UpdateAddHtlc]

    awaitCond(aliceHC.params.asInstanceOf[CustomCommitmentsPlugin].getHtlcsRelayedOut(Nil).values.flatten.map(_._2).toSet == Set(id3, id4, aliceAdd5.id)) // Pending outgoing is taken into account (we could have signed)
    ...
    awaitCond(aliceHC.params.asInstanceOf[CustomCommitmentsPlugin].getHtlcsRelayedOut(Nil).values.flatten.map(_._2).toSet == Set(id3, id4, aliceAdd5.id))

    bob ! CMD_FULFILL_HTLC(alice2bobUpdateAdd3.id, preimage3)
    alice ! bob2alice.expectMsgType[UpdateFulfillHtlc]

    awaitCond(aliceHC.params.asInstanceOf[CustomCommitmentsPlugin].getHtlcsRelayedOut(Nil).values.flatten.map(_._2).toSet == Set(id4, aliceAdd5.id)) // Fulfill in CLOSED is accepted

    bob ! CMD_FAIL_HTLC(alice2bobUpdateAdd4.id, Right(TemporaryNodeFailure))
    alice ! bob2alice.expectMsgType[UpdateFailHtlc]

    awaitCond(aliceHC.params.asInstanceOf[CustomCommitmentsPlugin].getHtlcsRelayedOut(Nil).values.flatten.map(_._2).toSet == Set(id4, aliceAdd5.id)) // Remote fail in CLOSED (disregarded)

    alice ! CurrentBlockCount(currentBlockHeight + 145)
    awaitCond(aliceHC.params.asInstanceOf[CustomCommitmentsPlugin].getHtlcsRelayedOut(Nil).values.flatten.map(_._2).isEmpty) // Local fake fail of outgoing payment on timeout
  }
}
